---
name: patent-infringement-check-archived-heavy
description: 【已归档 — 不要主动触发】这是 patent-infringement-check 的早期高 token 消耗历史版本（v5 重 reference 文档形态，11+ R-* flags / 19 类源 / 5 主体分类）的备份。当前生效版本是更轻量的 patent-infringement-check（Python 脚本切片 + Claude 主 agent + 每候选独立 react sub-agent；纯 Claude 实现，无外部 LLM）。除非用户明确要求 "用旧版" / "用高消耗版本" / "对照 v1"，否则**不要**触发此 skill。
---
...
3. **R-3 R-OPENSOURCE 双层激活**：上游开源项目 + 商业发行版**双层独立候选**——各自独立成行 / 各自独立跑 §A 协议
...
- **§C 子 agent 逐文档审阅**：每份本地归档证据**独立 spawn** general-purpose agent；禁止合并多份文档到同一 agent；并行 spawn。**所有候选只要存在归档证据均必跑**——不存在"低优先级候选可跳过 §C"的豁免
