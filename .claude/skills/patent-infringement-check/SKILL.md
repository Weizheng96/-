...
每一步要么落一份 markdown 报告到 `专利集/<PATENT_ID>/`，要么填充 `候选/<slug>/` 子目录。**所有 LLM 推理由 Claude 完成**——主 agent 负责 Steps 2-5，sub-agent 负责 Step 6 每个**被取证**候选（默认按侵权可能性排序的前 10）独立的取证+判定。Python 脚本仅做 HTTP 抓取、文件切片、模板汇总三类无 LLM 工作。
...
> **硬约束（防偷懒）：必须先把 Step 3/Step 5 的技术相关玩家枚举完整，再排序取前 N。** Step 6 默认只查前 10 是**预算控制**，不是允许在 Step 3/5 少列的借口——若枚举本身残缺，排序池就残缺，"前 10"只是"残缺清单里的前 10"，真正高侵权可能性的候选可能从没被列进来过。完整枚举 → 排序 → 取前 10，三步缺一不可。
...
**默认只取证侵权可能性最高的前 10 个候选（NN 01–10）**：Step 5 已按侵权可能性降序编好 NN，所以"前 10"= `候选/01-*` … `候选/10-*`。主 agent 默认**只为这 10 个** spawn sub-agent（分 2-3 批跑）；**NN 11 及以后暂不 spawn**，其 `_verdict.md` 保持 init 占位的"待评估"（Step 7 会归为"第 9 档"列末尾，属预期状态、非错误）。候选总数 ≤10 时全部取证。**仅当用户明确要求"继续 / 查更多 / 全部排查"时**，才按 NN 顺序续 spawn 后续批次（11–20、21–30……），同样每批 3-5 个、批内等待返回。这样把取证预算优先压在最可能侵权、最可能取到证据的候选上，其余按需扩查。**前提（与 Step 5 硬约束呼应）：前 10 的可信度依赖 Step 3/5 已完整枚举——不得因"只查前 10"而在 Step 3/5 偷懒少列，否则"前 10"失真。**
...
**Step 7 前置条件**：所有**已排查**候选（默认前 10，或用户已要求扩查到的范围）的 `_verdict.md` 必须含"第 N 档"字样（不停留在 init 占位）。**"默认前 10"模式下 NN 11+ 候选有意保持"待评估"——这是预期状态，不是需要回填的错误**；compile_step7.py 会把它们归为"第 9 档：待评估"列在末尾。仅当某个**本应已排查**（落在当前排查范围内）的候选仍为"待评估"时，才回 Step 6 补跑再 compile。

**主 agent 交付**：`Read` 输出确认；最终给用户这几个数字——
- 第 1+2 档候选数 = 高置信侵权候选
- 第 3 档数 = 建议法务深查
- 第 5 档数 = 已排除
- **第 9 档（待评估）数 = 超出默认前 10、尚未取证的候选**——并主动提示用户"如需继续排查后续候选，请告知，我再按侵权可能性顺序续跑"
