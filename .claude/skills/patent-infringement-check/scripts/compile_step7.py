...
def _classify_verdict(text: str) -> tuple[int, str]:
    ...
    if label == "确认侵权" and not rank:
        rank = 2
    elif label == "公开资料不足" and not rank:
        rank = 4
    elif label == "已排除" and not rank:
        rank = 5
    elif label == "待评估" or not (rank or label):
        return 9, "待评估"
    if not label:
        label = {1: "确认侵权（高）", 2: "确认侵权（中）",
                 3: "公开资料不足（强候选）", 4: "公开资料不足（弱候选）",
                 5: "已排除"}.get(rank, "未识别")
    return rank or 8, label
...
def main(argv=None):
    ...
    # Sort: rank ASC (1 best, 5 排除, 9 待评估 last); within rank by nn
    rows.sort(key=lambda r: (r["rank"], int(r["nn"]) if str(r["nn"]).isdigit() else 999))
    ...
    out.append("## 统计")
    rank_names = {1: "第 1 档（确认侵权·高）", 2: "第 2 档（确认侵权·中）",
                  3: "第 3 档（公开资料不足·强候选）", 4: "第 4 档（公开资料不足·弱候选）",
                  5: "第 5 档（已排除）", 8: "未识别", 9: "待评估"}
