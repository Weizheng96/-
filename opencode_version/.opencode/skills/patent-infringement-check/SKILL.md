...
**中性装配原则**：按候选 metadata 注入的内容只能是**中性的"待核查事实维度"，不得预告任何 F# 的三态方向或档位**。写"去查清 X 由什么触发 / Y 由什么决定 / 是否存在 Z 手段"，**不要**写"X 属某型 → 判该 F# 未命中 / 反据"——判定方向一旦写进 prompt，sub-agent 取证会退化成"印证既定结论"，并把主 agent 的一次主观判断复制到所有同类候选，破坏跨候选独立性与（对评测专利的）一致性。可提醒 sub-agent 遵循协议判定纪律里已有的通用栏杆（缺失→未确定 / 非互斥手段 / 等同信号，详见下方协议），但那是规则提醒、非本候选结论；同类候选判定不一致时靠 Step 6.2 复核跨候选对齐，不靠 prompt 统一。
> 中性 vs 带方向（跨领域示意）：化学配方 ✅ 查实际组分与配比 ／ ❌"用了替代溶剂 → 该组分未命中"；机械结构 ✅ 查连接方式与部件数 ／ ❌"焊接非螺栓 → 反据"；金融风控 ✅ 查触发条件与阈值来源 ／ ❌"人工审核触发 → 该信号未命中"。
