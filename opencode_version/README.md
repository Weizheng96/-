...
├── opencode.json             # opencode 配置：general subagent 权限（bash/edit/webfetch）+ 刻意不写死模型
├── .opencode/
│   └── skills/
...
- **脚本产物锚定修复**：5 个脚本的 `_find_repo_root()` 从「脚本自身位置」改为「`Path.cwd()`（打开的项目根）」向上找 `专利集/`。修复前，脚本会沿自身路径向上找，把产物写到父仓库 `专利集/`；修复后产物只落在打开的项目根下。**请始终把 `opencode_version` 作为项目根打开。**
- **读写限定项目内**：靠脚本 cwd 锚定 + skill 全程相对路径保证产物落在项目内；opencode 对项目外访问默认 `ask`（弹确认）。**注意**：不要用 `external_directory: "deny"` 做硬围栏——这版 opencode 用 `deny` 会连运行时读系统证书库一起挡掉，启动即报 `unknown certificate verification error`。
- **强制写盘**：SKILL.md 顶部 + AGENTS.md 加了硬规则——每步产物必须用工具写入磁盘文件，只在对话框输出 = 该步未完成（修复"opencode 只 narrate 不落盘"）。
- **自包含、可独立分享**：路径全相对、无父仓依赖；打包发给别人当独立项目根打开即可运行。
